...
# ── Apply shared figure style ─────────────────────────────────────────────
sys.path.insert(0, '/Users/ethan/Desktop')
from figure_style.style import apply_style, save_figure, get_colors, TOL_COLORS, FONT_LABEL
...
# ============================================================================
R_punch_actual = 0.010728  # mm (radius = 21.456 um diameter / 2)
...
def _subsample_even_r(r, *arrs, n=400):
    order = np.argsort(r)
    r_sorted = r[order]
    arrs_sorted = [a[order] for a in arrs]
    targets = np.linspace(r_sorted[0], r_sorted[-1], n)
    idx = np.clip(np.searchsorted(r_sorted, targets), 0, len(r_sorted) - 1)
    idx = np.unique(idx)
    return (r_sorted[idx], *(a[idx] for a in arrs_sorted))

fp_r_d, fp_uz_d, fp_szz_d = _subsample_even_r(fp_r, fp_uz, fp_szz)
fr_r_d, fr_uz_d, fr_szz_d = _subsample_even_r(fr_r, fr_uz, fr_szz)
...
with plt.rc_context({
    ...
    ax.plot(r_anal, fp_sned_full, '-', marker='^', markevery=40, markersize=10,
            linewidth=4, color=c[2], alpha=0.85, label='FP Sneddon')
    ...
    ax.plot(r_anal, fr_sned_full, '--', marker='D', markevery=40, markersize=10,
            linewidth=4, color=c[3], alpha=0.85,
            label='FEC Sneddon (a=b)')
...
# ---- Figure 2 (fig1 copy): same displacement profile with 1.6x larger fonts ----
def _scale_size(val, factor=1.6):
    return val * factor if isinstance(val, (int, float)) else val

_size_keys = ["font.size", "axes.titlesize", "axes.labelsize",
              "xtick.labelsize", "ytick.labelsize", "legend.fontsize"]
_big_sizes = {k: _scale_size(plt.rcParams[k]) for k in _size_keys}
...
with plt.rc_context({
    ...
    **_big_sizes,
    ...
    ax.set_title(r'\textbf{Displacement Profile at Interface}')
    ax.legend()
    _save_fig(fig, os.path.join(output_dir, "comparison_displacement_profile_fig1.pdf"))
