"""
Flat Cylindrical Punch — copied directly from Rocheville_Final_indent.ipynb
Only addition: saves interface profile data to results_flat_punch/ at the end.
"""
...
# Punch geometry
R_punch = 0.5          # mm radius (solve at convenient scale, rescale later)
H_punch = 1.0          # mm height

# Applied pressure
P_applied = 100000.0     # MPa applied to top of punch
...
output_dir = f"results_flat_punch_10xE"

print("Geometry parameters:")
print(f"  Substrate: R={R_substrate} mm, H={H_substrate} mm")
print(f"  Punch: R={R_punch} mm, H={H_punch} mm")
print(f"  Applied pressure: {P_applied} MPa")
...
gmsh.initialize()
gmsh.model.add("nanoindentation_3d_refined")
...
# Create punch cylinder on top of substrate
punch = gmsh.model.occ.addCylinder(
    0, 0, H_substrate,      # starts at top of substrate
    0, 0, H_punch,          # extends upward
    R_punch                 # smaller radius
)
...
gmsh.model.setPhysicalName(3, PUNCH_TAG, "Diamond_Punch")
...
for dim, tag in all_surfaces:
    ...
    # Top of punch (z ≈ H_substrate + H_punch, r < R_punch)
    if abs(z - (H_substrate + H_punch)) < 1e-3 and r < R_punch + 1e-3:
...
# Diamond punch
E_diamond = 12_000_000.0  # MPa 12000 GPa (10x baseline — closer to Sneddon's rigid-punch assumption)
...
print(f"  Traction: (0, 0, {-P_applied}) MPa")
...
A_punch = np.pi * R_punch**2
F_applied = P_applied * A_punch

print(f"Applied load:")
print(f"  Pressure: {P_applied} MPa")
print(f"  Area: {A_punch:.6f} mm²")
...
print("="*70)
print("SIMULATION SUMMARY - REFINED MESH")
print("="*70)
...
print("\n4. LOADING")
print(f"   Applied pressure: {P_applied} MPa")
print(f"   Applied force: {F_applied:.6f} N")
...
print("\n" + "="*70)
print("STATUS: ✓ REFINED MESH SIMULATION COMPLETE")
print("="*70)
...
# ============================================================================
# Radial grid for analytical solution (denser near punch)
# ============================================================================
# Use a two-part grid: dense near the punch, plus a coarser tail to r_max_fem
n_near = 400
n_far = 200
r_max_fem = float(r_profile_fem.max())
r_near_max = float(min(3.0 * R_punch, r_max_fem))

r_near = np.linspace(0.0, r_near_max, n_near)
if r_max_fem > r_near_max + 1e-12:
    r_far = np.linspace(r_near_max, r_max_fem, n_far)
    r_analytical = np.unique(np.concatenate([r_near, r_far]))
else:
    r_analytical = r_near

print(f"\nAnalytical grid:")
print(f"  r ∈ [0, {r_analytical.max():.3f}] mm with {len(r_analytical)} points")
...
# === Cell 15: Sneddon Analytical Solution Functions ===
# Sneddon analytical solution functions
# Using substrate properties only (steel)

# Material parameters (substrate only)
E_analytical = E_steel      # 200,000 MPa
eta_analytical = nu_steel   # 0.28
a_punch = R_punch          # 0.1 mm

# Compute shear modulus
mu_analytical = E_analytical / (2.0 * (1.0 + eta_analytical))

print("Sneddon Analytical Solution Parameters:")
print(f"  E = {E_analytical/1000:.0f} GPa (substrate only)")
print(f"  η = {eta_analytical}")
print(f"  μ = {mu_analytical:,.0f} MPa")
print(f"  a = {a_punch} mm (punch radius)")

# Extract D from the FEM simulation next

def sneddon_stress_under_punch(r, D, a, mu, eta):
    """
    Sneddon stress σ_zz under the punch (0 <= r < a)

    σ_zz(r, z=0) = -[(2*μ*D)/(π(1-η))] * (a² - r²)^(-1/2)
    """
    # Avoid singularity at r = a
    r_safe = np.minimum(r, a - 1e-10)

    # Compute stress
    factor = -(2.0 * mu * D) / (np.pi * (1.0 - eta))
    stress = factor / np.sqrt(a**2 - r_safe**2)

    return stress


def sneddon_displacement_outside_punch(r, D, a):
    """
    Sneddon displacement outside punch (r > a)

    u_z(r, 0) = [(2*D)/π] * arcsin(a/r)
    """
    # Ensure r > a to avoid domain error
    r_safe = np.maximum(r, a + 1e-10)

    # Compute displacement
    u_z = (2.0 * D / np.pi) * np.arcsin(a / r_safe)

    # Make negative (downward)
    return -u_z


print("\n✓ Sneddon analytical functions defined")

# === Cell 16+17: Sneddon Comparison Plots and Save Report ===
# Compute Sneddon analytical solutions using extracted D_fem
print("Computing Sneddon analytical solutions...")
print("="*70)

# Use the analytical grid defined earlier
r_under_punch = r_analytical[r_analytical < R_punch]
r_outside_punch = r_analytical[r_analytical > R_punch]

# Stress under punch (0 <= r < a)
sigma_zz_sneddon_under = sneddon_stress_under_punch(
    r_under_punch, D_fem, a_punch, mu_analytical, eta_analytical
)

# Displacement outside punch (r > a)
u_z_sneddon_outside = sneddon_displacement_outside_punch(
    r_outside_punch, D_fem, a_punch
)

# Force-displacement (Eq. 6.1)
F_sneddon = (4.0 * mu_analytical * a_punch * D_fem) / (1.0 - eta_analytical)

print(f"\n✓ Sneddon solutions computed")
print(f"  Stress points (under punch): {len(sigma_zz_sneddon_under)}")
print(f"  Displacement points (outside punch): {len(u_z_sneddon_outside)}")
print(f"  Sneddon predicted force (Eq 6.1): {F_sneddon:.6f} N")

# ============================================================================
# FOUR SEPARATE COMPARISON PLOTS
# ============================================================================
...
def _plot_stress_under_punch(ax):
    fem_mask_under = r_profile_fem < R_punch
    ax.plot(r_profile_fem[fem_mask_under], sigma_zz_fem[fem_mask_under], 'o',
            label='FEM (mesh nodes)', markersize=3, alpha=0.7, color='C0')
    ax.plot(r_under_punch, sigma_zz_sneddon_under, '-',
            label='Sneddon Analytical', linewidth=2, alpha=0.85, color='C1')
    ...
    ax.set_title('Stress Under Punch: FEM vs Sneddon')
    ax.legend(fontsize=9)
    ax.grid(True, alpha=0.3)


def _plot_displacement_outside_punch(ax):
    fem_mask_outside = r_profile_fem > R_punch
    ax.plot(r_profile_fem[fem_mask_outside], u_z_fem[fem_mask_outside], 'o',
            label='FEM (mesh nodes)', markersize=3, alpha=0.7, color='C0')
    ax.plot(r_outside_punch, u_z_sneddon_outside, '-',
            label='Sneddon Analytical', linewidth=2, alpha=0.85, color='C1')
    ax.axvline(R_punch, color='k', linestyle=':', alpha=0.5, label='Punch edge')
    ax.set_xlabel('Radial distance r (mm)')
    ax.set_ylabel('$u_z$ (mm)')
    ax.set_title('Displacement Outside Punch: FEM vs Sneddon')
    ax.legend(fontsize=9)
    ax.grid(True, alpha=0.3)


def _plot_full_displacement(ax):
    ax.plot(r_profile_fem, u_z_fem, 'o',
            label='FEM (mesh nodes)', markersize=2.5, alpha=0.7, color='C0')

    u_z_sneddon_full = np.zeros_like(r_analytical)
    mask_under = r_analytical < R_punch
    mask_outside = r_analytical > R_punch

    u_z_sneddon_full[mask_under] = -D_fem
    u_z_sneddon_full[mask_outside] = sneddon_displacement_outside_punch(
        r_analytical[mask_outside], D_fem, a_punch
    )

    ax.plot(r_analytical, u_z_sneddon_full, '-',
            label='Sneddon Analytical', linewidth=2, alpha=0.85, color='C1')

    ax.axvline(R_punch, color='k', linestyle=':', alpha=0.5, label='Punch edge')
    ax.set_xlabel('Radial distance r (mm)')
    ax.set_ylabel('$u_z$ (mm)')
    ax.set_title('Full Displacement Profile')
    ax.legend(fontsize=9)
    ax.grid(True, alpha=0.3)


def _plot_force_displacement(ax):
    ax.plot([D_fem], [F_applied], 'ro', markersize=9,
            label=f'FEM (applied): F={F_applied:.2f} N')
    ax.plot([D_fem], [F_sneddon], 'kx', markersize=9,
            label=f'Sneddon (Eq 6.1): F={F_sneddon:.2f} N')
    ax.plot([0.0, D_fem], [0.0, F_sneddon], 'k--', alpha=0.35, linewidth=1)

    ax.set_xlabel('Indentation depth D (mm)')
    ...
    param_text = (
        f"P = {P_applied} MPa\n"
        f"a = {R_punch} mm\n"
        f"E = {E_steel/1000:.0f} GPa\n"
        f"$\\eta$ = {nu_steel}\n"
        f"D_interface = {D_fem:.3e} mm\n"
        f"D_global = {D_global:.3e} mm"
    )
...
stress_plot_file = os.path.join(output_dir, "sneddon_stress_under_punch.pdf")
disp_outside_plot_file = os.path.join(output_dir, "sneddon_displacement_outside_punch.pdf")
disp_full_plot_file = os.path.join(output_dir, "sneddon_displacement_full.pdf")
force_plot_file = os.path.join(output_dir, "sneddon_force_displacement.pdf")

fig, ax = plt.subplots(figsize=(7.5, 5.5))
_plot_stress_under_punch(ax)
_save_fig(fig, stress_plot_file)

fig, ax = plt.subplots(figsize=(7.5, 5.5))
_plot_displacement_outside_punch(ax)
_save_fig(fig, disp_outside_plot_file)

fig, ax = plt.subplots(figsize=(7.5, 5.5))
_plot_full_displacement(ax)
_save_fig(fig, disp_full_plot_file)

fig, ax = plt.subplots(figsize=(7.5, 5.5))
_plot_force_displacement(ax)
_save_fig(fig, force_plot_file)

print("\n✓ Sneddon comparison plots generated")

# Save comprehensive summary report
print("Saving summary report...")
print("="*70)

# Sneddon force prediction (Eq. 6.1)
F_sneddon = (4.0 * mu_analytical * a_punch * D_fem) / (1.0 - eta_analytical)
...
params_dict = {
    ...
    "sneddon_parameters": {
        "E_analytical_MPa": E_analytical,
        "eta_analytical": eta_analytical,
        "mu_analytical_MPa": float(mu_analytical),
        "a_punch_mm": a_punch,
        "D_used_mm": float(D_fem),
        "F_predicted_N": float(F_sneddon)
    },
    "output_files": {
        "plots": {
            "stress_under_punch": os.path.basename(stress_plot_file),
            "displacement_outside_punch": os.path.basename(disp_outside_plot_file),
            "displacement_full": os.path.basename(disp_full_plot_file),
            "force_displacement": os.path.basename(force_plot_file)
        }
    }
}
...
np.savetxt(
    ...
    comparison_data,
    delimiter=',',
    header='r_mm,u_z_mm,sigma_zz_MPa',
    comments=''
)
...
print("\n" + "="*70)
print("✓ FLAT PUNCH SIMULATION COMPLETE")
print("="*70)
